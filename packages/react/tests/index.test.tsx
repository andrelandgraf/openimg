...
test("generates <source> elements for each breakpoint and format", () => {
  const { container } = render(<Img src="/cat.png" width={800} height={800} alt="A cute cat" />);
...
test("verifies srcset strings and img src for /cat.png with 300w 300h", () => {
  const { container } = render(<Img src="/cat.png" width={300} height={300} alt="A small cat" />);
  ...
  formats.forEach((format) => {
    expectedBreakpoints.forEach(({ width, height }) => {
      ...
      const expectedSrcset = `/img/cat.png?w=${width}&h=${height}&format=${format}`;
      expect(source.getAttribute("srcset")).toBe(expectedSrcset);
    });
  });

  // Verify the fallback img tag src
  const expectedImgSrc = `/img/cat.png?w=300&h=300`;
